...
from Main_OPSH_3 import Ui_MainWindow
from History_OPSH_2 import Ui_History_Windows

from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.QtWidgets import QApplication, QMessageBox, QAbstractItemView, QTableWidgetItem, QMainWindow, QTableWidget  
...
class MainWindow(QtWidgets.QMainWindow,Ui_MainWindow): 
    ...
    def style_table(self):
        self.tableWidget_BD.setColumnWidth(0,420) # Установить длинну столбца 1
        self.tableWidget_BD.setColumnWidth(1,298) # Установить длинну столбца 2
        self.tableWidget_BD.horizontalHeader().setStretchLastSection(True) # Установить длинну столбца 3 на всю оставшуюся ширину окна
...
##############################################################--Функции кнопок в программе--##################################################################
    ...
    def take_flag(self):
        try:
            ...
            try:
                ...
                cur.execute(sql_request) # Выполненеие запроса
                ...
                try:
                    ...
                    try:
                        cur.execute(sql_request2) # Выполненеие запроса
    ...
    def butt_add(self): 
        if len(self.lineEdit_name.text())>0: # Проверка, чтобы наименование товара не было пустым
            if (self.T_check_in('prod', self.lineEdit_name.text(), self.lineEdit_date.text())) == False: # Проверка на дубликаты
                try:
                    ...
                    if datetime.strptime(d2, '%Y-%m-%d') > datetime.strptime('2000-01-01', '%Y-%m-%d'):
                        ...
                        self.T_select_table('prod', 'date') # Вывод всех данных таблицы с сортировко по порядку добавления 
...
###############################################################--Функции для работы с таблицами--#################################################################
    ...
    def T_text_search(self, name_table, text, methods='date'):
        if text == '#Delete all data prod': 
            if self.show_messagebox_delteall() == True: # При положительном ответе на вопрос
    ...
    # Функция для проверки наличия (одиниковых) данных уже в БД 
        ...
        if name_table == 'prod': 
            try:
                ...
                try:
                    ...
                    conn.commit()
                    print("В таблицу {0} доблены новые значения!".format(name_table))
                except:
    ...
    def update_table(self, data):
        ...
        for nam, i in enumerate(data):
            ...
            elif Day_difference <= 30 and Day_difference >= 1 and i[4] == 0 : # Жельтым: до окончания срока годности 30 дней
...
class WinHist(QtWidgets.QWidget, Ui_History_Windows): 
    ...
    def style_table_h(self):
        self.tableWidget.setColumnWidth(0,250) # Установить длинну столбца 1
        self.tableWidget.setColumnWidth(1,150) # Установить длинну столбца 2
        self.tableWidget.setColumnWidth(2,150) # Установить длинну столбца 2
        self.tableWidget.horizontalHeader().setStretchLastSection(True) # Установить длинну столбца 4 на всю оставшуюся ширину окна
...
#``````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````
    ...
    def update_table_h(self, data):
        ...
        for nam, i in enumerate(data):
            ...
            elif Day_difference <= 10 and Day_difference >= 1: #Желтым: до окончания срока годности 10 дней
            ...
            elif Dayend_difference <= -180: #Окончательно удалить все элементы, которые были удалены более 180 дней готорых больше 180
                data180 = True
                print ('Найден 180+') #Здесь необходимо удалить элемент!!!!!!!!!!!!!!!!!!!!!!!!! #Dayend_difference
...
# Удалить любую таблицу и всё ее содержимое
# def T_drop_table(name_table):
#     try:
#         sql_request_temp = """DROP TABLE {name_tables};"""
#         sql_request = sql_request_temp.format(name_tables=name_table)
#         cur.execute(sql_request)
#         conn.commit()
#         print('Таблица '+ name_table +' удалена!')
#     except:
#         print('Проверьте правильность наименования таблицы!')
...
# Переопределение функции преобразования к нижнему геристру
...
conn.create_collation("NOCASE", ignore_case_collation) # Для работы с регистрами кирилицы 
conn.create_function("LOWER", 1, sqlite_lower) # Для работы с регистрами кирилицы 
conn.create_function("UPPER", 1, sqlite_upper) # Для работы с регистрами кирилицы 
